...
```pulse
ghost
fn elim_is_tree_nil (#t:Type0) (x:tree_t t)
requires is_tree x 'l ** pure ('l == T.Leaf) //elim is like from the LHS, we are getting the RHS
...
```pulse
ghost
fn intro_is_tree_nil (#t:Type0) (x:tree_t t) // opposite to elim. elim is breakig, intro is assembling.
...
{
  rewrite each ft as (T.Node data ltree rtree);
  rewrite_by
    (is_tree ct (T.Node data ltree rtree))
    (exists* (p:node_ptr t) (lct:tree_t t) (rct:tree_t t).
      pure (ct == Some p) **
      pts_to p { data = data; left = lct; right = rct } **
      is_tree lct ltree **
...
```pulse
ghost
fn intro_is_tree_cons (#t:Type0) (ct:tree_t t) (v:node_ptr t) (#node:node t) (#ltree:T.tree t) (#rtree:T.tree t)
requires
  pts_to v node **
...
{
    rewrite (pts_to v node) as (pts_to v { data=node.data; left = node.left; right = node.right});
    rewrite_by //fold is_tree ct (T.Node node.data ltree rtree) ---> This works now. Rebuild and see.
        (exists* (p:node_ptr t) (lct:tree_t t) (rct:tree_t t).
          pure (ct == Some p) **
          pts_to p { data = node.data; left = lct; right = rct } **
          is_tree lct ltree **
          is_tree rct rtree)
          (is_tree ct (T.Node node.data ltree rtree))
           vprop_equiv_norm
...
ensures  is_tree_cases x ft
{
  match ft {
    ...
    T.Node data ltree rtree -> { 
      elim_is_tree_cons x ft data ltree rtree;
      with w lct rct. _;
      let v = Some?.v x;
      rewrite each w as v;
      rewrite each lct as (({ data = data; left = lct; right = rct }).left) in (is_tree lct ltree);
      rewrite each rct as (({ data = data; left = lct; right = rct }).right) in (is_tree rct rtree);
...
{
  cases_of_is_tree x l;
  rewrite each x as (None #(ref (node t)));
  unfold (is_tree_cases None l);
  intro_is_tree_nil x;
}
...
ensures is_tree x 'l ** pure (n == T.height 'l)
{
   match x {
    ...
    Some vl -> {
      ...
      let left_tree_height = height node.left;
      let right_tree_height = height node.right;
...
```
```pulse
fn rec height_subtree (#t:Type0) (x:tree_t t) (k:nat)
requires is_tree x 'l
returns n:nat
ensures is_tree x 'l ** pure (n == k + T.height 'l)
{
  match x {
    ...
    Some vl -> {
      ...
      let l1 = height n.left;
      let l2 = height n.right;
      let n1 = height_subtree n.left (1 + k);
      let n2 = height_subtree n.right (1 + k);
      intro_is_tree_cons x vl;
      
      (*if let l1 = height n.left;
           let l2 = height n.right; is written after intro_is_tree_cons error is resulted. Why?*)
      if (l1 > l2)
      {
         n1
      }
...
```
let null_tree_t #t : tree_t t = None #(node_ptr t)
...
{
    intro_is_tree_nil #t null_tree_t;
    null_tree_t #t
}
```
```pulse
fn node_cons (#t:Type) (v:t) (ltree:tree_t t) (rtree:tree_t t) 
...
```pulse
fn rec rotate_left (#t:Type0) (x:tree_t t)
requires is_tree x 'l ** (pure (Some? (T.rotate_left 'l)))
        (pure (Some? x)) **
        (pure (Some? rtr)) ** pure (rtree == T.Node data_r ltree_r rtree_r) 
returns y:tree_t t 
ensures exists* (ft:T.tree t).
  pure (Some ft == T.rotate_left 'l) ** (is_tree y ft)
{
